...
I chose to measure my time in minutes. Note that instead of plotting total amount of time spent over the two weeks, I decided to plot average time spent on these activities per weekday, so that I could look at what I would expect my time distribution to be for a single week. For example, if I spent 120 minutes in total watching anime on Mondays, I would only plot 120/2 = 60 minutes for Monday (Note: since I observed collected data for 3 Fridays for starting and ending on a Friday, Friday total time would be divided by 3 instead of 2). We can first observe what I intended to accomplish.
